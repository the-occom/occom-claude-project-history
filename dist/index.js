    ...
    if (mode === "--serve") {
        ...
        app.post("/api/workflows", express.json(), async (req, res) => {
            try {
                ...
                await db.query(`INSERT INTO workflows (id, name, description) VALUES ($1, $2, $3)
           ON CONFLICT (id) DO UPDATE SET name = $2, description = COALESCE($3, workflows.description), updated_at = NOW()`, [id, name, description ?? null]);
                res.json({ ok: true });
            }
            catch (e) {
        ...
        app.get("/hooks/active-tasks", async (req, res) => {
            try {
                const wid = req.query.workflow_id;
                if (!wid) {
                    res.status(400).json({ error: "workflow_id required" });
                    return;
                }
                const result = await db.query(`SELECT COUNT(*) as count FROM tasks WHERE workflow_id = $1 AND status = 'in_progress'`, [wid]);
        ...
        app.get("/hooks/context-inject", async (req, res) => {
            try {
                ...
                if (!wid) {
                    res.type("text/plain").send("[cph] No workflow detected for this project.\nCall cph_workflow_create to set up project memory.");
                    return;
                }
                const [workflowResult, tasksResult, blockersResult, decisionsResult, sessionInitResult] = await Promise.all([
                    db.query(`SELECT name, status FROM workflows WHERE id = $1`, [wid]),
                    db.query(`SELECT title, status FROM tasks WHERE workflow_id = $1 AND status IN ('in_progress', 'pending')
             ORDER BY CASE status WHEN 'in_progress' THEN 0 WHEN 'pending' THEN 1 END,
             ${PRIORITY_ORDER}
             LIMIT 5`, [wid]),
                    db.query(`SELECT title, blocker_type FROM blockers WHERE workflow_id = $1 AND status = 'open'
             ORDER BY opened_at ASC LIMIT 3`, [wid]),
                    db.query(`SELECT title FROM decisions WHERE workflow_id = $1 ORDER BY created_at DESC LIMIT 3`, [wid]),
                    sid
                        ? db.query(`SELECT 1 as found FROM tool_events WHERE session_id = $1 AND tool_name LIKE '%session_init%' LIMIT 1`, [sid])
        ...
        app.post("/hooks/attach-commit", express.json(), async (req, res) => {
            try {
                ...
                if (!workflow_id || !commit_hash) {
                    res.status(400).json({ error: "workflow_id and commit_hash required" });
                    return;
                }
             ...
             RETURNING id
           ) SELECT COUNT(*) as count FROM updated`, [commit_hash, diff_stat ?? null, workflow_id]);
        ...
        app.post("/hooks/task-complete", express.json(), async (req, res) => {
            try {
                ...
                const { rows } = await db.query(`SELECT status, updated_at FROM tasks WHERE id = $1`, [task_id]);
                if (!rows[0]) {
                ...
                await db.query(`UPDATE tasks SET status = 'completed', completed_at = NOW(), updated_at = NOW(),
           actual_minutes = COALESCE($3, actual_minutes),
           completion_notes = COALESCE($4, completion_notes)
           WHERE id = $1 AND updated_at = $2`, [task_id, rows[0].updated_at, actual_minutes ?? null, completion_notes ?? null]);
        ...
        app.post("/hooks/task-cancel", express.json(), async (req, res) => {
            try {
                ...
                await db.query(`UPDATE tasks SET status = 'cancelled', completion_notes = $1, completed_at = NOW(), updated_at = NOW()
    ...
    switch (event) {
        ...
        case "PreToolUse":
            if (data.tool_name === "TodoWrite" && data.todos) {
                await syncTodos(sessionId, workflowId, data.todos, timestamp, db);
            }
        ...
        case "PostToolUseFailure":
         ...
         VALUES ($1,$2,$3,'failure',$4,$5,$6,$7)`, [newId(), sessionId, workflowId, data.tool_name, data.error_type, data.interrupted, timestamp]);
        ...
        case "PreCompact":
         ...
         VALUES ($1,$2,$3,$4,$5)`, [newId(), sessionId, workflowId, data.trigger, timestamp]);
            break;
        default:
            await db.query(`INSERT INTO tool_events (id, session_id, workflow_id, phase, tool_name, created_at)
         VALUES ($1,$2,$3,'signal',$4,$5)`, [newId(), sessionId, workflowId, event, timestamp]);
    }
}
async function syncTodos(sessionId, workflowId, todos, timestamp, db) {
    if (!workflowId)
        return;
    for (const todo of todos) {
        const existing = await db.query(`SELECT id, status FROM tasks
       WHERE session_id = $1 AND title = $2 AND from_plan = true`, [sessionId, todo.content]);
        if (existing.rows.length === 0) {
            await db.query(`INSERT INTO tasks
         (id, workflow_id, session_id, title, status, from_plan, created_at, updated_at)
         VALUES ($1,$2,$3,$4,$5,true,$6,$6)`, [newId(), workflowId, sessionId, todo.content,
            ...
            if (row.status !== newStatus) {
                await db.query(`UPDATE tasks SET status = $1, updated_at = $2 WHERE id = $3`, [newStatus, timestamp, row.id]);
                if (newStatus === "in_progress") {
                    await db.query(`UPDATE tasks SET started_at = $1 WHERE id = $2 AND started_at IS NULL`, [timestamp, row.id]);
                }
                if (newStatus === "completed") {
                    await db.query(`UPDATE tasks SET completed_at = $1 WHERE id = $2`, [timestamp, row.id]);
                }
            }
        }
    }
}
function mapTodoStatus(s) {
    return s === "completed" ? "completed"
        : s === "in_progress" ? "in_progress"
            : "pending";
}
async function maybeSealPlanPhase(workflowId, timestamp, db) {
    if (!workflowId)
        return;
    // Only seal once — clear last_planning_started_at on first write
    await db.query(`UPDATE workflows SET last_planning_started_at = NULL
     WHERE id = $1 AND last_planning_started_at IS NOT NULL`, [workflowId]);
}
main().catch((error) => {
