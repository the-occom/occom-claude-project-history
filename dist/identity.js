 ...
 */
export async function upsertDeveloper(identity, db) {
    await db.query(`INSERT INTO developers (id, name, first_seen_at, last_seen_at, session_count)
     VALUES ($1, $2, NOW(), NOW(), 1)
     ON CONFLICT (id) DO UPDATE
       SET last_seen_at = NOW(),
           session_count = developers.session_count + 1,
           name = COALESCE(NULLIF($2, ''), developers.name)`, [identity.id, identity.name]);
}
