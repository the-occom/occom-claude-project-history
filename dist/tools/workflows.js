...
Returns: Created workflow with ID. PUT THIS ID IN YOUR CLAUDE.md.`,
        ...
        try {
            ...
            const id = newId();
            await db.query(`INSERT INTO workflows (id, name, description, git_branch_pattern)
           VALUES ($1, $2, $3, $4)`, [id, name, description ?? null, git_branch_pattern ?? null]);
            const wf = await findOne(db, `SELECT * FROM workflows WHERE id = $1`, [id]);
            return {
                content: [{
                        type: "text",
                        text: JSON.stringify({
...
Returns: Array of workflows with ID, name, status. No task counts (use cph_workflow_summary for that).`,
        ...
        try {
            const db = await getDb();
            const result = status
                ? await db.query(`SELECT * FROM workflows WHERE status = $1 ORDER BY updated_at DESC`, [status])
                : await db.query(`SELECT * FROM workflows ORDER BY updated_at DESC`);
...
Use this when a user explicitly asks for project status. Don't call proactively.`,
        ...
        try {
            const db = await getDb();
            const wf = await findOne(db, `SELECT * FROM workflows WHERE id = $1`, [workflow_id]);
            if (!wf) {
            ...
            const [counts, blockers, decisions, accuracy] = await Promise.all([
                db.query(`SELECT status, COUNT(*) as count FROM tasks WHERE workflow_id = $1 GROUP BY status`, [workflow_id]),
                db.query(`SELECT COUNT(*) as count FROM blockers WHERE workflow_id = $1 AND status = 'open'`, [workflow_id]),
                db.query(`SELECT COUNT(*) as count FROM decisions WHERE workflow_id = $1`, [workflow_id]),
                db.query(`SELECT ROUND(AVG(CAST(actual_minutes AS float) / NULLIF(estimated_minutes, 0))::numeric, 2)::text as ratio
             FROM tasks
             WHERE workflow_id = $1
               AND actual_minutes IS NOT NULL
               AND estimated_minutes IS NOT NULL
               AND estimated_minutes > 0`, [workflow_id])
            ]);
    ...
    server.registerTool("cph_workflow_update", {
        ...
        try {
            ...
            if (git_branch_pattern !== undefined) {
                fields.push(`git_branch_pattern = $${idx++}`);
                values.push(git_branch_pattern);
            }
            if (fields.length === 1) {
