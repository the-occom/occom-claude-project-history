...
unless the user explicitly asks. Pull individual records on demand with their ID.`,
        ...
        try {
            ...
            if (gitContext.engineer_id) {
                const pref = await findOne(db, `SELECT * FROM engineer_preferences WHERE engineer_id = $1`, [gitContext.engineer_id]);
                if (pref)
                    resolvedDepth = pref.retrieval_depth;
            }
...
Returns either a matched workflow or a suggestion to create one.`,
        ...
        try {
            ...
            const workflows = await db.query(`SELECT * FROM workflows WHERE status = 'active' AND git_branch_pattern IS NOT NULL`);
...
deep     = + teammate activity + historical patterns (use when debugging complex issues)`,
        ...
        try {
            ...
            const id = newId();
            await db.query(`INSERT INTO engineer_preferences (id, engineer_id, retrieval_depth)
           VALUES ($1, $2, $3)
           ON CONFLICT (engineer_id) DO UPDATE SET retrieval_depth = $3, updated_at = NOW()`, [id, resolvedId, depth]);
            return {
...
For project context, use cph_session_init.`,
        ...
        try {
            ...
            const activeWfs = await db.query(`SELECT COUNT(*) as count FROM workflows WHERE status = 'active'`);
            return {
                content: [{
                        type: "text",
                        text: JSON.stringify({
