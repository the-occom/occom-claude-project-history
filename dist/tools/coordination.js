...
export function registerCoordinationTools(server) {
    server.registerTool("cph_who_is_working", {
...
Use this to understand team state before starting work.`,
        ...
        try {
           ...
           ORDER BY s.started_at DESC`, params);
            const agents = result.rows.map(r => {
                let status = "available";
                if (r.current_blocker_id)
                    status = "blocked";
                else if (r.current_task_status === "in_progress")
                    status = "active";
                else if (r.current_task_id)
                    status = "idle_with_task";
                return {
    ...
    server.registerTool("cph_activity_stream", {
...
Filter by workflow, time range, and event types.`,
        ...
        try {
            ...
            values.push(limit);
            const result = await db.query(`SELECT a.id, d.name AS developer_name, a.event_type,
