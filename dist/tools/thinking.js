...
export function registerThinkingTools(server) {
    server.registerTool("cph_thinking_summary", {
...
Args:
  ...
  - limit: Number of recent turns to return (default 10)`,
        ...
        try {
            ...
            values.push(limit);
            const estimates = await db.query(`SELECT * FROM thinking_estimates ${where}
           ORDER BY created_at DESC LIMIT $${idx}`, values);
            const baselines = await db.query(`SELECT * FROM tool_baselines ORDER BY sample_count DESC`);
            // Compute aggregates
            const rows = estimates.rows;
            const totalThinkingMs = rows.reduce((sum, r) => sum + (r.initial_gap_ms ?? 0) + r.interleaved_ms, 0);
