  ...
  router.get("/tasks/:id", async (req, res) => {
    try {
      ...
      const [subtasks, blockers, decisions] = await Promise.all([
        db.query(
          "SELECT id, title, status, priority, estimated_minutes, actual_minutes FROM tasks WHERE parent_task_id = $1 ORDER BY created_at",
          [req.params.id]
        ),
        db.query(
          "SELECT id, title, blocker_type, status, opened_at, resolved_at, resolution, resolution_minutes FROM blockers WHERE task_id = $1 ORDER BY opened_at",
          [req.params.id]
        ),
        db.query(
          "SELECT id, title, decision, tags, created_at FROM decisions WHERE task_id = $1 ORDER BY created_at",
          [req.params.id]
        ),
      ]);
