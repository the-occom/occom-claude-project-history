...
interface SessionState {
  ...
  pendingDeltas: ContextDelta[];
  lastSyncAt: Date;
}

const TOKEN_BUDGETS: Record<RetrievalDepth, number> = {
  minimal: 300,
  standard: 600,
  deep: 1200,
};
...
export function registerSession(sessionId: string): void {
  ...
  bus.addListener(sessionId, (event: ChangeEvent) => {
    const s = sessions.get(sessionId);
    if (!s) return;
    if (s.workflowId && event.workflow_id !== s.workflowId) return;
...
function evictDeltas(deltas: ContextDelta[], budget: number): ContextDelta[] {
...
function scoreDelta(delta: ContextDelta): number {
  let score = 0;

  // Recency: up to 40 pts, decays per hour
  const ageHours =
    (Date.now() - new Date(delta.timestamp).getTime()) / (1000 * 60 * 60);
  score += Math.max(0, 40 - ageHours);

  // Inserts are more important than updates
  if (delta.op === "INSERT") score += 20;
  if (delta.op === "DELETE") score += 15;

  // Tasks and blockers are higher priority
  if (delta.table === "tasks" || delta.table === "blockers") score += 10;
