...
async function getWorkflowSummary(
  ...
  const result = await db.query<Workflow>(
...
async function getActiveTasks(
  ...
  const result = await db.query<Pick<Task, "id" | "title" | "priority" | "status">>(
...
async function getOpenBlockers(
  ...
  const result = await db.query<Pick<Blocker, "id" | "title" | "blocker_type">>(
    `SELECT id, title, blocker_type
     FROM blockers
...
async function getRelevantDecisions(
  ...
  recentFiles: string[],
  limit: number
): Promise<Pick<Decision, "id" | "title" | "decision">[]> {
  if (!recentFiles.length) {
    // Fall back to most recent decisions
    const result = await db.query<Pick<Decision, "id" | "title" | "decision">>(
  ...
  if (!tokens.length) {
    const result = await db.query<Pick<Decision, "id" | "title" | "decision">>(
      `SELECT id, title, decision FROM decisions WHERE workflow_id = $1 ORDER BY created_at DESC LIMIT $2`,
      [workflowId, limit]
    );
    return result.rows;
  }
  ...
  const conditions = tokens
    ...
    .map(
      (_, i) =>
        `(title ILIKE $${i + 3} OR tags ILIKE $${i + 3} OR decision ILIKE $${i + 3})`
    )
    .join(" OR ");

  const values: unknown[] = [workflowId, limit, ...tokens.slice(0, 5).map((t) => `%${t}%`)];
  ...
  if (!result.rows.length) {
    const fallback = await db.query<Pick<Decision, "id" | "title" | "decision">>(
      `SELECT id, title, decision FROM decisions WHERE workflow_id = $1 ORDER BY created_at DESC LIMIT $2`,
      [workflowId, limit]
    );
...
async function getTeammateActivity(
  db: PGlite,
  workflowId: string,
...
function buildSessionHint(
  ...
  if (openBlockers.length) {
    parts.push(
      `${openBlockers.length} open blocker${openBlockers.length > 1 ? "s" : ""} need attention`
    );
  }
  ...
  if (!parts.length) return "No active work. Start a task to begin tracking.";
