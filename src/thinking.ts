...
function getTurn(sessionId: string): TurnState | undefined {
  return turns.get(sessionId);
}

// ── Exports ──────────────────────────────────────────────────────────────────

export function onUserPrompt(sessionId: string, timestamp: string): void {
...
export async function onPreToolUse(
  ...
  if (turn?.lastToolEventId && turn.lastPostTimestamp) {
    ...
    await db.query(
      `UPDATE tool_events SET gap_after_ms = $1 WHERE id = $2`,
      [gapMs, turn.lastToolEventId]
    );
  }
...
export function onPostToolUseFailure(sessionId: string, timestamp: string): void {
  const turn = getTurn(sessionId);
  if (turn) {
    turn.lastPostTimestamp = timestamp;
  }
...
export async function sealTurnThinking(
  sessionId: string,
  workflowId: string | null,
  timestamp: string,
  db: PGlite
  ...
  const turn = getTurn(sessionId);
  if (!turn || turn.toolEventIds.length === 0) return;
  ...
  const result = await db.query<{
    ...
    `SELECT id, pre_timestamp, post_timestamp, execution_ms
  ...
  await db.query(
    `INSERT INTO thinking_estimates
...
function msGap(from: string, to: string): number {
  return Math.max(0, new Date(to).getTime() - new Date(from).getTime());
}

async function updateBaselines(eventIds: string[], db: PGlite): Promise<void> {
  ...
  for (const row of result.rows) {
    await db.query(
      `INSERT INTO tool_baselines (tool_name, avg_ms, p50_ms, p95_ms, sample_count, updated_at)
       VALUES ($1,$2,$3,$4,$5,NOW())
       ON CONFLICT (tool_name) DO UPDATE
       SET avg_ms = $2, p50_ms = $3, p95_ms = $4, sample_count = $5, updated_at = NOW()`,
      [row.tool_name, row.avg_ms, row.p50_ms, row.p95_ms, row.cnt]
    );
  }
}
