...
Returns: Created task. Call cph_task_start immediately after.`,
    ...
    async ({ workflow_id, title, description, parent_task_id, priority, estimated_minutes }) => {
      try {
        ...
        const id = newId();
        await db.query(
          `INSERT INTO tasks (id, workflow_id, parent_task_id, title, description, priority, estimated_minutes)
...
Call this only when explicitly asked — session_init already provides active tasks.`,
    ...
    async ({ workflow_id, status, include_subtasks, limit, offset }) => {
      try {
        ...
        if (!include_subtasks){ conditions.push(`parent_task_id IS NULL`); }

        const where = conditions.length ? `WHERE ${conditions.join(" AND ")}` : "";
        values.push(limit, offset);

        const result = await db.query<Pick<Task, "id" | "title" | "status" | "priority" | "estimated_minutes" | "actual_minutes">>(
    ...
    async ({ task_id }) => {
      try {
        ...
        const [subtasks, blockers] = await Promise.all([
          db.query<Pick<Task, "id" | "title" | "status" | "priority">>(
            `SELECT id, title, status, priority FROM tasks WHERE parent_task_id = $1 ORDER BY created_at`,
            [task_id]
          ),
          db.query(
            `SELECT id, title, blocker_type, status, opened_at FROM blockers WHERE task_id = $1 ORDER BY opened_at`,
            [task_id]
          )
        ]);
