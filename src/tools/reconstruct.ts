...
export function registerReconstructTools(server: McpServer): void {
    ...
    async ({ workflow_id }) => {
      try {
        ...
        const [workflow, tasks, decisions, blockers, sessions, activity] = await Promise.all([
          db.query(
            `SELECT id, name, status, created_at, updated_at FROM workflows WHERE id = $1`,
            [workflow_id]
          ),
          db.query(
            `SELECT id, title, status, priority, estimated_minutes, actual_minutes,
          ...
          db.query(
            `SELECT id, title, decision, rationale, alternatives_considered,
          ...
          db.query(
            `SELECT id, title, blocker_type, status, resolution,
          ...
          db.query(
            `SELECT s.id, s.started_at, s.ended_at, s.exit_reason,
          ...
          db.query(
            `SELECT a.event_type, a.subject_type, a.subject_title, a.created_at,
        ...
        const cancelledTasks = allTasks.filter((t: any) => t.status === "cancelled");
        const resolvedBlockers = blockers.rows.filter((b: any) => b.status === "resolved");

        const deadEnds = [
          ...cancelledTasks.map((t: any) => ({
            type: "cancelled_task",
            id: t.id,
            title: t.title,
        ...
        for (const s of sessions.rows as any[]) {
          if (s.developer_id) devMap.set(s.developer_id, s.developer_name);
        }
        const developers = Array.from(devMap.entries()).map(([id, name]) => ({ id, name }));

        // Date range
        const allDates = [
          ...(allTasks.map((t: any) => t.started_at).filter(Boolean)),
          ...(sessions.rows.map((s: any) => s.started_at).filter(Boolean)),
        ];
