...
export function registerCodebaseTools(server: McpServer, sessionId: string): void {
...
Upserts by workflow_id + path_pattern — safe to call multiple times.`,
    ...
    async ({ workflow_id, areas }) => {
      try {
        ...
        const results: Array<{ path_pattern: string; action: string }> = [];

        for (const area of areas) {
          const existing = await db.query<{ id: string }>(
            `SELECT id FROM file_areas WHERE workflow_id = $1 AND path_pattern = $2`,
            [workflow_id, area.path_pattern]
          );

          if (existing.rows.length > 0) {
            await db.query(
              `UPDATE file_areas SET
